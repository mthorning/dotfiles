---
name: pr-workflow
description: Create, inspect, or update GitHub PRs associated with the current Jujutsu branch/bookmark. Use when the user asks to create a PR, view a PR, fetch PR metadata/comments, or update the PR description. Can also accept a GitHub PR URL to work with any PR from any repository.
---

# PR Creation, Inspection & Update

Create new PRs, view PR details and existing review comments, and update PR descriptions. Works with either the current Jujutsu bookmark or any GitHub PR URL.
...
- Scripts must be run using their absolute path: `~/dotfiles/skills/pr-workflow/scripts/<script-name>`
- This skill is for **PR workflow management and inspection**, not for performing an independent code review of a PR's changes.
...
```bash
bash ~/dotfiles/skills/pr-workflow/scripts/get_pr_reviews.sh
```

View PR from GitHub URL:
```bash
bash ~/dotfiles/skills/pr-workflow/scripts/get_pr_reviews.sh "https://github.com/owner/repo/pull/123"
```
...
```bash
bash ~/dotfiles/skills/pr-workflow/scripts/update-pr.sh
```

Update specific PR by URL:
```bash
bash ~/dotfiles/skills/pr-workflow/scripts/update-pr.sh "https://github.com/owner/repo/pull/123"
```

Create new PR (always in draft mode):
```bash
echo "<body>" | bash ~/dotfiles/skills/pr-workflow/scripts/update-pr.sh --create --title "type(scope): short description"
```
...
10. For **create**: pipe body to script with `--title` and, when needed, `--base`:
   ...
   ```
   ```bash
   echo "<body>" | bash ~/dotfiles/skills/pr-workflow/scripts/update-pr.sh --create --title "<title>" --base "<base-branch>"
   ```
   For **update**: pipe body to script:
   ```bash
   echo "<body>" | bash ~/dotfiles/skills/pr-workflow/scripts/update-pr.sh
   ```
