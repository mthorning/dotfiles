local function setup_lsp()
  ...
  local function setup_prettier_on_save(bufnr)
    if vim.fn.exists(':Prettier') == 0 then
      return
    end

    local group = vim.api.nvim_create_augroup('PrettierOnSave.' .. bufnr, { clear = true })
  ...
  vim.lsp.config['eslint'] = {
    ...
    before_init = function(_, config)
      ...
      local flat_configs = vim.tbl_filter(function(f)
        return f:match('config')
      ...
      for _, f in ipairs(flat_configs) do
        ...
        found = vim.tbl_filter(function(p)
          return not p:find('[/\\]node_modules[/\\]')
        end, found)
        if #found > 0 then
  ...
  vim.api.nvim_create_autocmd({ 'TextChanged', 'TextChangedI' }, {
    pattern = '*',
    callback = function(event)
      check_and_update_diagnostics(event.buf)
    end,
  })
